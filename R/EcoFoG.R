...
TricoterMemo <- function (destination="docs") {
  ...
  # Knit to HTML
  options(knitr.table.format='html')
  rmarkdown::render(input="memo.Rmd", output_format=bookdown::html_document2(), output_dir=destination)
  # Knit to pdf
  options(knitr.table.format='latex')
  rmarkdown::render(input="memo.Rmd", output_format=bookdown::pdf_book(base_format=EcoFoG::memo), output_dir=destination)
  # Copy to destination
  docsDirs <- list.dirs(path=destination, full.names=TRUE, recursive=TRUE)
  dir.create(paste(OriginalWD, "/", destination, sep=""), showWarnings=FALSE)
